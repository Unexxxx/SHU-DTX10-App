...
typedef NS_ENUM(NSInteger, ThingRSAKeySize) {
    ThingRSAKey512Bits  = 512,     // 512位
    ThingRSAKey1024Bits = 1024,    // 1024位
    ThingRSAKey2048Bits = 2048     // 2048位
};

// 本类只用于加密/解密Data类型的数据，入参和返回值转换由上层类(NSString+ThingSecurity\NSData+ThingSecurity)实现
...
/**
 ...
 @param config 加密配置，默认可以用+ [ThingRSAConfig defaultConfig]
 @return 加密后的数据
 */
...
/**
 ...
 @param config 解密配置，默认可以用+ [ThingRSAConfig defaultConfig]
 @return 解密后的数据
 */
